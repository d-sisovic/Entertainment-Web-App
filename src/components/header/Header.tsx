...
const categoryStyles = {
    [Category.ALL]: styles["icons--active"],
    [Category.MOVIE]: styles["icons--active"],
    [Category.TV_SERIES]: styles["icons--active"],
    [Category.BOOKMARK]: styles["icons--active"],
};

const Header = () => {
    const { category, setCategory } = useAppStore().movies;

    const currentStyles = categoryStyles[category] || "";
    ...
    desktop:min-w-24 desktop:max-h-[60rem] desktop:flex-col desktop:py-8 desktop:px-7 desktop:rounded-[1.25rem] desktop:m-0 desktop:mt-8 desktop:ml-8 ${styles.navbar}`}>
        ...
        <div className={`${styles.icons} flex gap-6 justify-between h-4 tablet:h-5 tablet:gap-8 desktop:flex-col desktop:mb-auto desktop:mt-[4.688rem] desktop:gap-10`}>
            <img src={imageHome} alt="home" className={currentStyles} onClick={() => onSelectCategory(Category.ALL)} />

            <img src={imageMovie} alt="movie" className={currentStyles} onClick={() => onSelectCategory(Category.MOVIE)} />

            <img src={imageTvSeries} alt="tv series" className={currentStyles} onClick={() => onSelectCategory(Category.TV_SERIES)} />

            <img src={imageBookmark} alt="bookmark" className={currentStyles} onClick={() => onSelectCategory(Category.BOOKMARK)} />
        </div>
